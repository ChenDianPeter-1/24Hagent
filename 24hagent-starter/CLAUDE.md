...
Claude Code is the executor. Codex is the controller.

Claude Code may perform bounded work requested by Codex:
- code search
- file inspection
...
Claude Code must follow the exact task packet from Codex. Do not expand scope on your own.
...
1. Read the relevant section of this `CLAUDE.md` and the original task packet again.
2. Add or tighten a short rule in this `CLAUDE.md` only when Codex explicitly asks you to do so.
3. Then implement the bounded repair.

Do not self-edit `CLAUDE.md` for every revision. Only update it when the revision reflects a repeatable Claude Code behavior problem or Codex explicitly requests a rule update. Keep new rules short, concrete, and project-specific.
