...
// src/core/quality/validation-engine.ts
function parseCoverageFromRawOutput(rawOutput) {
...
function parseCoveragePyJson(raw) {
  ...
  if (typeof totals.num_branches === "number" && totals.num_branches > 0) {
    branches = (totals.covered_branches ?? 0) / totals.num_branches * 100;
  }
  return { lines, branches, functions: null, statements: lines, profile: "coverage_py" };
}
function evaluateThresholds(coverage, thresholds) {
  const metrics = [
...
async function runConfiguredGates(plans, runner) {
  const results = [];
...
function evaluateGateResults(results, configs) {
...
function renderValidationReport(params) {
  const { verdict, taskId, timestamp, readinessVerdict, fileScopeCheck } = params;
  const readinessNote = readinessVerdict === "BLOCKED" ? "BLOCKED - quality gates cannot execute until tooling is configured." : "Project tooling is configured.";
  const gateRows = verdict.gateResults.map((r) => {
    const exit = r.exitCode !== null ? String(r.exitCode) : "N/A";
    const detail = r.status === "PASS" ? "Exit code 0" : r.status === "SKIPPED" ? "Gate disabled in config" : r.rawOutput.length > 500 ? r.rawOutput.slice(0, 500) + "...(truncated)" : r.rawOutput;
...
var CoverageThresholdSchema, GateConfigSchema, QualityGatesSchema, GATE_ORDER;
var init_validation_engine = __esm({
  "src/core/quality/validation-engine.ts"() {
    "use strict";
    init_zod();
    CoverageThresholdSchema = external_exports.object({
      lines: external_exports.number(),
      branches: external_exports.number().nullable(),
      functions: external_exports.number().nullable(),
      statements: external_exports.number()
    });
    GateConfigSchema = external_exports.object({
    ...
    QualityGatesSchema = external_exports.object({
      gates: external_exports.object({ test: GateConfigSchema, lint: GateConfigSchema, typecheck: GateConfigSchema, coverage: GateConfigSchema })
    });
    GATE_ORDER = ["test", "lint", "typecheck", "coverage"];
  }
});
...
// src/core/review/result-renderer.ts
function renderReviewMarkdown(params) {
  const { reviewResult: r, taskId, timestamp, rawOutputPath } = params;
  const blockingSection = r.blocking_issues.length > 0 ? r.blocking_issues.map(
    (b) => `- id: ${b.id}
  severity: ${b.severity}
  issue: ${b.issue}
  evidence: ${b.evidence}
  ...
  const fixesSection = r.required_fixes.length > 0 ? r.required_fixes.map((f) => `- ${f}`).join("\n") : "(none)";
  const suggestionsSection = r.non_blocking_suggestions.length > 0 ? r.non_blocking_suggestions.map((s) => `- issue: ${s.issue}
  rationale: ${s.rationale}`).join("\n") : null;
  const questionsSection = r.human_questions.length > 0 ? r.human_questions.map(
...
var init_result_renderer = __esm({
  "src/core/review/result-renderer.ts"() {
    "use strict";
  }
});
...
// src/core/schemas/review-result.ts
...
function parseCodexJsonlToReviewResult(rawJsonl) {
  ...
  const raw = (0, import_yaml.parse)(yamlBlock);
  return ReviewResultSchema.parse(raw);
}
var import_yaml, BlockingIssue, NonBlockingSuggestion, HumanQuestion, ReviewResultSchema;
var init_review_result = __esm({
  "src/core/schemas/review-result.ts"() {
    "use strict";
    init_zod();
    import_yaml = __toESM(require_dist(), 1);
    BlockingIssue = external_exports.object({
      id: external_exports.string(),
      severity: external_exports.literal("BLOCKING"),
      issue: external_exports.string(),
      evidence: external_exports.string(),
      required_fix: external_exports.string()
    });
    NonBlockingSuggestion = external_exports.object({
      issue: external_exports.string(),
      rationale: external_exports.string()
    });
    HumanQuestion = external_exports.object({
      question: external_exports.string(),
      options: external_exports.array(external_exports.string())
    });
    ReviewResultSchema = external_exports.object({
      schemaVersion: external_exports.string().default("1.0"),
      verdict: external_exports.enum(["PASS", "NEED_FIX", "NEED_HUMAN"]),
      confidence: external_exports.enum(["high", "medium", "low"]),
      blocking_issues: external_exports.array(BlockingIssue),
      required_fixes: external_exports.array(external_exports.string()),
      non_blocking_suggestions: external_exports.array(NonBlockingSuggestion).default([]),
      human_questions: external_exports.array(HumanQuestion).default([]),
      next_action: external_exports.enum(["continue_next_task", "fix_current_task", "ask_human"])
    });
  }
});
...
if (cmd === "status") {
  ...
  const { runReadiness: runReadiness2 } = await Promise.resolve().then(() => (init_readiness(), readiness_exports));
  runReadiness2(".");
} else if (cmd === "validate") {
  const { runValidate: runValidate2 } = await Promise.resolve().then(() => (init_validate(), validate_exports));
  runValidate2(".");
